...
var GitFlowVisualize =
        ...
        (function () {
                ...
                for (var i = 0; i < data.chronoCommits.length; i++) {
                    ...
                    if (!commit.columns) {
                        if (regexMerge.test(commit.message)) {
                            putCommitInColumn(commit.id, 'd0');
                        }
                    }
                }
            };
            var isolateIntegration = function () {
                var head = $.grep(data.branches, function (item) { return (item.id == options.integrationRef); });
                if (head.length == 0) return;

                var versionCommitPath = findIntegrationPathFrom(head[0].latestChangeset);
                for (var i = 0; i < versionCommitPath.length; i++) {
                    putCommitInColumn(versionCommitPath[i], 'i0', data);
                }
                // find extra develop commits that are on secondary develop columns
                var integrationBranch = options.integrationRef.substring(options.integrationRef.lastIndexOf('/') + 1);
                var regexMerge = new RegExp("Merge branch '[^']+' (of \\S+ )?into " + integrationBranch);
                for (var i = 0; i < data.chronoCommits.length; i++) {
                    var commit = data.commits[data.chronoCommits[i]];
                    if (!commit.columns) {
                        if (regexMerge.test(commit.message)) {
                            putCommitInColumn(commit.id, 'i0');
                        }
                    }
                }
            };
            ...
            var findIntegrationPathFrom = function (from) {
                var integrationBranch = options.developRef.substring(options.developRef.lastIndexOf('/') + 1);
                var regexSelfMerge = new RegExp("Merge branch '(" + integrationBranch + ")' of http:\\/\\/\\S+ into \\1");
                var regexRealMerge = new RegExp("Merge branch '[^']+' into " + integrationBranch);
                var score = function (path, nextId) {
                    var c = data.commits[nextId];
                    var last = data.commits[path.last()];
                    // no part of m or d can be i
                    if (c.columns && (c.columns[0] == 'm' || c.columns[0] == 'd')) return false;
                    // next commit cannot have a child further down the line
                    var childrenInPath = c.children.filter(function (child) {
                        return child in path.members;
                    });
                    if (childrenInPath.length != 1) return false;
                    // merges of integration onto itself are neutral
                    if (regexSelfMerge.test(c.message))
                        return 0;
                    //merges of an evol branch onto integration are a big bonus
                    if (regexRealMerge.test(c.message))
                        return 20;
                    // following first parent is a bonus
                    if (last.parents.length > 1 && c.id == last.parents[0].id) return 1;
